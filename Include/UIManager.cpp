...
#else
	printf("********************************\n");
	printf("func='%s' not support this os.\n",__FUNCTION__);
	printf("********************************\n");
#endif
}
...
#else
	ASSERT(/* !::IsBadWritePtr(pstrModuleName,-1)||!::IsBadReadPtr(pstrModuleName,-1) ||*/ pstrModuleName == NULL );
#endif
...
#else
				(PFUNCSETLAYEREDWINDOWATTR)::GetProcAddress(hUser32, TEXT("SetLayeredWindowAttributes"));
#endif
    ...
    case WM_PAINT:
        {
            ...
            if( m_bOffscreenPaint )
            {
                ...
                for( int i = 0; i < m_aPostPaintControls.GetSize(); i++ ) {
					...
					__Trace(_T("*************\ncontrol name='%s'.\n"),pPostPaintControl->GetName().GetData());
