...
public:
    ...
    virtual void DrawItemText(HDC hDC, const RECT& rcItem,RECT* pPaintItem) = 0;
};
...
public:
	...
	//rcConrolItem为控件区域大小，rcItem画图区域
    void DrawItemText(HDC hDC, const RECT& rcItem,RECT* pPaintItem);
};
...
public:
    ...
    void DrawItemText(HDC hDC, const RECT& rcItem,RECT* pPaintItem);    
    void DrawItemBk(HDC hDC, const RECT& rcItem);
	void SetListExPos(RECT rc)
	{

		CContainerUI::SetPos(rc);
		if( m_pOwner == NULL ) return;		

		if (m_pListExHeader == NULL)
		{
			return;
		}
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			CListHeaderItemUI *pHeaderItem = static_cast<CListHeaderItemUI*>(m_pListExHeader->GetItemAt(i));
