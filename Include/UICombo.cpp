...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	case 'd':
		...
		else if( _tcscmp(pstrName, _T("dropbox")) == 0 ) {SetDropBoxAttributeList(pstrValue);return ;}
		 break;
	case 'h':
		if( _tcscmp(pstrName, _T("focusedimage")) == 0 ) {SetFocusedImage(pstrValue);return ;}
		else if( _tcscmp(pstrName, _T("hotimage")) == 0 ) {SetHotImage(pstrValue);return ;}
		break;
	case 'i':
		...
		case 's':
			...
			else if( _tcscmp(pstrName, _T("itemshowhtml")) == 0 ) {SetItemShowHtml(_tcscmp(pstrValue, _T("true")) == 0);return ;}
			break;
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 ) {
        ...
        if( pElement != NULL ) {
            pElement->DrawItemText(hDC, rcText,&rcText);
        }
        else {
